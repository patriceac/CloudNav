[CmdletBinding()]
param(
    [ValidateSet('Ui', 'Migration', 'MigrationResume', 'OneDrive', 'Failure', 'WindowPosition')]
    [string[]]$Scenario = @('Ui', 'Migration', 'MigrationResume', 'OneDrive', 'Failure', 'WindowPosition'),
    [switch]$SkipBuild,
    [switch]$Force
)
...
$cases = @{
    ...
    Migration = @{ Exe = 'CloudNav.exe'; Args = '--demo-migration "{OUTDIR}\migration-result.json"'; Actions = 'vm-migration-actions.json'; Result = 'migration-result.json'; Images = @('migration-copy-progress.png','migration-verify-progress.png','migration-verified.png','migration-cutover.png') }
