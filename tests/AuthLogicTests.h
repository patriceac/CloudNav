...
inline void TestAuthLogic() {
    ...
    assert(AuthFailureDetails("googleapi: Error 403: Quota exceeded, RATE_LIMIT_EXCEEDED") ==
        L"Google Drive's API quota is temporarily exhausted. Try again later.");
    ...
    for (const auto* name : {"config_is_local", "config_refresh_token", "config_drive_ok"}) {
        automatic.clear();
    ...
    automatic.clear();
    assert(!AutomaticAuthAnswer(SyncJson{{"Name", "config_shared_client_id"}}, false, "", automatic));
